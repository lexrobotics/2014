...
task arm() {
	...
	while(true){
		...
		if(joy2Btn(6))//if left front trigger is pressed, flagMotor is powered, spinning in order to turn the flag raising thing
			motor[flagMotor] = -100;
		else if(joy2Btn(8))//if left back trigger is pressed, flagMotor is powered, spinning in the opposite direction
			motor[flagMotor] = 100;
		else
			motor[flagMotor] = 0;
		...
		if(joy2Btn(4))//retracts the flag raiser
			servo[flagExtender] = 255;
		else if(joy2Btn(2))//extends the flag raiser
